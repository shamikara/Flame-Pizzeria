...
CREATE TABLE `cateringrequest` (
    ...
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `userId` INTEGER NULL,
    `quoteSubtotal` DOUBLE NULL,
    `quoteServiceCharge` DOUBLE NULL,
    `quoteTax` DOUBLE NULL,
    `quoteTotal` DOUBLE NULL,
    `depositDue` DOUBLE NULL,
    `paymentStatus` VARCHAR(191) NOT NULL DEFAULT 'UNPAID',
    `paymentIntentId` VARCHAR(191) NULL,
    `paymentClientSecret` VARCHAR(191) NULL,
